...
import com.mt.helper.OrderDetail;
import com.mt.helper.OutgoingReqInterceptor;
import com.mt.helper.SumTotalOrder;
...
public class OrderTest {
    ...
    public void shop_create_then_confirm_payment_for_an_order() throws InterruptedException {
        ...
        Assert.assertEquals(HttpStatus.OK, exchange.getStatusCode());
        Assert.assertNotNull(exchange.getHeaders().getLocation().toString());
        Thread.sleep(20*1000);//wait for order create/update
        ...
        log.debug("order id {}",getOrderId(exchange.getHeaders().getLocation().toString()));
