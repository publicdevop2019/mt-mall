...
public class ConcludeOrderDTXRepresentation extends CommonDTXRepresentation {
    ...
    public ConcludeOrderDTXRepresentation(DistributedTx var0) {
        setId(var0.getId());
        setStatus(var0.getStatus());
        setChangeId(var0.getChangeId());
        setOrderId(var0.getLockId());
        setCreatedAt(var0.getCreatedAt().getTime());
        statusMap.put(DECREASE_ACTUAL_STORAGE_LTX, var0.getLocalTxStatusByName(DecreaseActualStorageForConcludeEvent.name));
        statusMap.put(UPDATE_ORDER_LTX, var0.getLocalTxStatusByName(UpdateOrderForConcludeEvent.name));
        emptyOptMap.put(DECREASE_ACTUAL_STORAGE_LTX, var0.isLocalTxEmptyOptByName(DecreaseActualStorageForConcludeEvent.name));
        emptyOptMap.put(UPDATE_ORDER_LTX, var0.isLocalTxEmptyOptByName(UpdateOrderForConcludeEvent.name));
