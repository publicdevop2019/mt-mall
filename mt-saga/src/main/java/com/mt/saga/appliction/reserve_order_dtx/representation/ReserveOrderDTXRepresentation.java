...
public class ReserveOrderDTXRepresentation extends CommonDTXRepresentation {
    ...
    public ReserveOrderDTXRepresentation(DistributedTx var0) {
        setId(var0.getId());
        setStatus(var0.getStatus());
        setChangeId(var0.getChangeId());
        setOrderId(var0.getLockId());
        setCreatedAt(var0.getCreatedAt().getTime());
        statusMap.put(UPDATE_ORDER_LTX, var0.getLocalTxStatusByName(UpdateOrderForReserveEvent.name));
        statusMap.put(DECREASE_ORDER_STORAGE_LTX, var0.getLocalTxStatusByName(DecreaseOrderStorageForReserveEvent.name));
        emptyOptMap.put(UPDATE_ORDER_LTX, var0.isLocalTxEmptyOptByName(UpdateOrderForReserveEvent.name));
        emptyOptMap.put(DECREASE_ORDER_STORAGE_LTX, var0.isLocalTxEmptyOptByName(DecreaseOrderStorageForReserveEvent.name));
