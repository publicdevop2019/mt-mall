package com.mt.user_profile.application.cart;
...
import com.mt.user_profile.application.cart.command.*;
...
import com.mt.user_profile.domain.cart.CartItemId;
import com.mt.user_profile.domain.cart.CartQuery;
import com.mt.user_profile.domain.cart.event.*;
...
public class CartApplicationService {
        ...
        try {
            ...
            String bizCart = template.execute(new TransactionCallback<String>() {
                ...
                public String doInTransaction(TransactionStatus status) {
                    return ApplicationServiceRegistry.getIdempotentService().idempotent(changeId,
                            (change) -> {
    ...
    public void handle(ClearCartForCreateEvent event) {
        ...
        ApplicationServiceRegistry.getIdempotentService().idempotentMsg(changeId, (change) -> {
            try {
    ...
    public void cancelClearCart(CancelClearCartForCreateCommand event) {
        ApplicationServiceRegistry.getIdempotentService().idempotentMsg(event.getChangeId(), (change) -> {
            CartQuery cartQuery = new CartQuery(event);
            try {
