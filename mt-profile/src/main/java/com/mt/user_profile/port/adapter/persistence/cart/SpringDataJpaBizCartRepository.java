...
import com.mt.user_profile.domain.cart.BizCart_;
...
public interface SpringDataJpaBizCartRepository extends JpaRepository<BizCart, Long>, BizCartRepository {
    ...
    private QueryUtility.QueryContext<BizCart> getBizCartQueryContext(CartQuery cartQuery) {
        ...
        Optional.ofNullable(cartQuery.getCartItemIds()).ifPresent(e -> QueryUtility.addDomainIdInPredicate(e.stream().map(DomainId::getDomainId).collect(Collectors.toSet()), BizCart_.CART_ITEM_ID, queryContext));
        Order order = null;
